...
* **Added** | "Performance & Latency" section in Settings: one-click real-time priority for the WiVRn server (CAP_SYS_NICE) with live status, plus a compact list of latency tips (5 GHz WiFi, hardware encoder, H.265/AV1, foveated encoding, GPU power profile). Automatically disabled on Flatpak/Nix where setcap can't work.
...
* **Neu** | „Performance & Latenz“-Bereich in den Settings: Echtzeit-Priorität für den WiVRn-Server (CAP_SYS_NICE) per Klick mit Live-Status, dazu kompakte Latenz-Tipps (5-GHz-WLAN, Hardware-Encoder, H.265/AV1, Foveated Encoding, GPU-Powerprofil). Bei Flatpak/Nix, wo setcap nicht greift, automatisch deaktiviert.
